...
const researchProduct = createStep({
  id: 'research-product',
  description: 'Product Analyst agent researches the product website and produces a thorough markdown summary',
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    return { url, researchSummary: response.text };
  },
});

const discoverOpportunities = createStep({
  id: 'discover-opportunities',
  description: 'Discovery agent takes the research summary and searches HN for competitors, pain points, and opportunities',
  inputSchema: z.object({
    url: z.string(),
    researchSummary: z.string(),
  }),
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    const response = await agent.generate(
      ...
      { structuredOutput: { schema: discoveryResultSchema }, maxSteps: 10 },
    );

    return { url, researchSummary, discovery: response.object ?? null };
  },
});
